...
const VideoPlayer = () => {
    ...
    const deleteVideo = async () => {
        try {
            const res = await fetch('https://video-uploader-api.vercel.app/delVideo/'+ id,{
                method: 'DELETE',
            });
        } catch (error) {
  ...
  return (
    <VideoPlayerStyled>
        <div className='del-and-back-container'>
            <div className="back">
                <Link to={'/'}><i className="fas fa-arrow-left"></i>Back to Videos</Link>
            </div>
            <div className="back">
                <Link onClick={() => deleteVideo()} to={'/'}><i className="fas fa fa-trash"></i> 
                    Delete Video
                </Link>
            </div>
        </div>
